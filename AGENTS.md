...
En todos los módulos, `application.port.in` y `application.port.out` contienen únicamente interfaces; no existe un paquete genérico `application.model`. Los adaptadores son infraestructura: los controladores OpenAPI residen en `infrastructure.input.web` y los tipos jOOQ solo se consumen desde `infrastructure.output.persistence.jooq` del módulo propietario. La configuración Spring, las properties tipadas y los componentes propios de Spring Security residen bajo `infrastructure`; no se colocan en `application` ni `domain`. `@Transactional` está permitido en servicios de aplicación para delimitar casos de uso. Esta estructura está aplicada y protegida por los gates conforme a `ADR-0026`.

Antes de crear, renombrar o mover cualquier clase Java debe usarse la Skill local `$arquitectura-hexagonal-y-modulith`. La Skill exige clasificar responsabilidad, módulo, paquete, dependencias y prueba antes de editar, y una revisión posterior mediante otro agente cuando la capacidad de subagentes esté disponible. La revisión no sustituye ArchUnit, Spring Modulith ni los gates aplicables.
...
`api/openapi/running-coach.yaml` es la fuente de verdad OpenAPI 3.1. Los tipos generados de servidor y cliente se escriben solo bajo `build/generated/openapi`; los primeros solo podrán consumirse desde `infrastructure.input.web`. La API inicial permanece bajo `/api`, no contiene `paths` funcionales ni endpoints de salud públicos y declara sesión opaca como seguridad heredada. CSRF se exige además, en la misma alternativa de seguridad, solo en operaciones protegidas que modifican estado.

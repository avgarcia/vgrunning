...
5. `ADR-0011` define la entrega asíncrona mediante outbox, worker interno y Brevo. El refinamiento propuesto en `ADR-0021` comprueba que el corredor continúa activo antes de cada intento y omite sin reintentos al inactivo.
...
| `RF-08` | Clasificación, Planificación y Publicación | Un grupo activo combina segmentos e inclusiones o exclusiones persistentes; una reconfiguración protege traslados y la candidatura confirma miembros activos antes de congelarlos. | `D-01` | `ADR-0005` a `ADR-0007`, `ADR-0019`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-08`; pruebas de fórmula, estados de grupo, reconfiguración, candidatura, exclusividad y captura al publicar. | Diseñado en [publicación](phase-2-detailed-design-publication.md); pendiente validar `ADR-0021` |
| `RF-09` | Publicación | Validar, versionar y activar plan y destinatarios dentro de una única transacción con coordinación de grupos, revisiones y fecha local. | `D-01`, `D-06` | `ADR-0007`, `ADR-0019`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-09`; pruebas de fallo sin visibilidad parcial, candidatura obsoleta y carreras con clasificación, grupos o fecha. | Diseñado en [publicación](phase-2-detailed-design-publication.md); pendiente validar `ADR-0021` |
| `RF-10` | Publicación | Cada versión conserva contenido completo, nombre de grupo y destinatarios congelados desde la primera publicación; cambios posteriores no recalculan el conjunto. | `D-01`, `D-06` | `ADR-0007`, `ADR-0019`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-10`; pruebas ante cambios posteriores de contenido, actividad, grupo o clasificación. | Diseñado en [publicación](phase-2-detailed-design-publication.md); pendiente validar `ADR-0021` |
...
| `RF-13` | Planificación y Portal del corredor | Cada entrenamiento declara modalidad; la ubicación libre opcional se conserva y solo se admite en presencial. | `D-04` | `ADR-0006` (Aceptado), `ADR-0020` (Aceptado) | Criterios de `RF-13`; pruebas de captura, ausencia, rechazo en línea y consulta posterior. | Aportación de planificación validada; experiencia del portal pendiente |
| `RF-14` | Planificación y Publicación | Estados visibles `borrador` y `publicado`; tras publicar no hay borrador persistente, retirada ni restauración y nombre, grupo y semana quedan fijos. | `D-06` | `ADR-0006`, `ADR-0007`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-14`; pruebas de transición irreversible, concurrencia y ausencia de cambios pendientes o restauración. | Diseñado en planificación y [publicación](phase-2-detailed-design-publication.md); pendiente validar `ADR-0021` |
| `RF-15` | Publicación y Entrega de notificaciones | Una sesión local puede cambiar varios días futuros y los confirma como una versión completa y una tanda de notificaciones para destinatarios congelados. | `D-06` | `ADR-0007`, `ADR-0008`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-15`; pruebas temporales, varios días, versión única, grupo inactivo, atomicidad y notificación. | Diseñado en [publicación](phase-2-detailed-design-publication.md); entrega base aceptada y elegibilidad pendiente de validar |
| `RF-16` | Portal del corredor; Identidad y acceso | Vista móvil del único plan semanal propio, sus fases, bloques, objetivos relativos, modalidad y ubicación, sin exponer datos ajenos. | `D-04`, `D-08` | `ADR-0002`, `ADR-0004`, `ADR-0006` (Aceptado), `ADR-0020` (Aceptado) | Criterios de `RF-16`; pruebas adaptables, de estructura, representación de objetivos y aislamiento. | Modelo de planificación validado; portal del corredor pendiente |
...
| `RF-20` | Publicación y Entrega de notificaciones | Cada versión genera una solicitud individual por destinatario, resumen semanal y días cambiados; antes de cada intento se omite al corredor inactivo y se conserva el orden. | `D-06` | `ADR-0007`, `ADR-0008`, `ADR-0011` (Aceptados), `ADR-0021` (Propuesto) | Criterios de `RF-20`; pruebas de contenido, actividad, omisión, atomicidad, orden y carrera con baja. | Diseñado en [publicación](phase-2-detailed-design-publication.md); pendiente validar `ADR-0021` y controles previos a producción |
...
| `D-06` | Borrador solo antes de publicar; después, sesión local y sustitución atómica de días futuros, versiones completas, destinatarios congelados y solicitud transaccional de correo. | `ADR-0007`, `ADR-0008`, `ADR-0011`, `ADR-0020` (Aceptados), `ADR-0021` (Propuesto) |
...
| `ADR-0021`: edición de publicaciones y elegibilidad de notificaciones | Regla temporal, sesión local de varios días, ausencia de retirada y de borrador persistente, autoría mínima y omisión de correo a inactivos. | Implementación de `publication` hasta validación humana; datos personales reales y producción continúan bloqueados. | Revisor de arquitectura | Propuesto con decisiones funcionales resueltas; debe validarse antes de sustituir normativamente las partes incompatibles de ADRs aceptados. |
...
- Una edición publicada podría alterar días ya alcanzados o acumular un borrador distinto de lo visible. Mitigación: `ADR-0021` propuesto usa fecha local del servidor, bloquea hoy y pasado y confirma contenido y versión en una sola transacción.

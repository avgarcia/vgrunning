...
| Plan semanal y entrenamiento | Un grupo tiene como máximo un plan por semana. El plan agrupa como máximo un entrenamiento completo por día; cada entrenamiento contiene modalidad, calentamiento, bloques principales con objetivo, enfriamiento y lugar libre opcional solo en presencial. |
...
| `RF-04` | Planificación; Clasificación y segmentación | Modalidad del corredor como etiqueta; modalidad propia del entrenamiento y ubicación libre opcional solo en presencial. | `D-02`, `D-04` | `ADR-0005`, `ADR-0006` (Aceptado), `ADR-0019`, `ADR-0020` (Propuesto) | Criterios de `RF-04`; pruebas de modalidad, valores permitidos, ubicación ausente y rechazo en línea. | Clasificación validada y planificación documentada en [Diseño detallado de planificación](phase-2-detailed-design-planning.md), pendiente de aceptar `ADR-0020` |
...
| `RF-07` | Planificación | Cada grupo tiene como máximo un plan por semana; el plan admite como máximo un entrenamiento completo por día de lunes a domingo. | — | `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-07`; pruebas de ciclo de vida, semana, revisión global, unicidad grupo-semana y unicidad diaria. | Documentado en [Diseño detallado de planificación](phase-2-detailed-design-planning.md), pendiente de aceptar `ADR-0020` |
| `RF-08` | Clasificación, Planificación y Publicación | Un grupo activo combina segmentos e inclusiones o exclusiones persistentes; una reconfiguración multigrupo protege traslados y la primera publicación congela miembros. | `D-01` | `ADR-0005` (Aceptado), `ADR-0006` (Aceptado), `ADR-0007` (Aceptado), `ADR-0019` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-08`; pruebas de fórmula, estados de grupo, reconfiguración, exclusividad y captura al publicar. | Clasificación validada y planificación documentada; diseños detallados de publicación y aceptación de `ADR-0020` pendientes |
| `RF-09` | Publicación | Validar, versionar y activar plan y destinatarios dentro de una única transacción con coordinación de grupos y revisión del borrador. | `D-01`, `D-06` | `ADR-0007` (Aceptado), `ADR-0019` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-09`; pruebas de fallo sin visibilidad parcial y carreras con clasificación, grupos o borrador. | Garantías de clasificación y planificación documentadas; diseño detallado de publicación pendiente |
| `RF-10` | Publicación | Cada publicación conserva contenido inmutable, nombre de grupo congelado y destinatarios congelados desde la primera versión. | `D-01`, `D-06` | `ADR-0007` (Aceptado), `ADR-0019` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-10`; pruebas ante cambios posteriores del borrador, nombre de grupo o clasificación. | Aportación de planificación documentada; diseño detallado de publicación pendiente |
| `RF-11` | Planificación | El tipo de la parte principal usa el catálogo cerrado; calentamiento y enfriamiento son siempre `rodaje`. | — | `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-11`; pruebas de catálogo, fases fijas y entrenamiento completo. | Documentado en [Diseño detallado de planificación](phase-2-detailed-design-planning.md), pendiente de aceptar `ADR-0020` |
| `RF-12` | Planificación | Bloques y recuperaciones usan magnitudes acotadas; los objetivos son una zona `Z1..Z5` o distancia de referencia con intervalo `-60..+180 s/km`. | — | `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-12`; pruebas de escalas, unidades, familias, bloques y recuperación tras cada repetición. | Documentado en [Diseño detallado de planificación](phase-2-detailed-design-planning.md), pendiente de aceptar `ADR-0020` |
| `RF-13` | Planificación y Portal del corredor | Cada entrenamiento declara modalidad; la ubicación libre opcional se conserva y solo se admite en presencial. | `D-04` | `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-13`; pruebas de captura, ausencia, rechazo en línea y consulta posterior. | Aportación de planificación documentada; experiencia del portal pendiente |
| `RF-14` | Planificación y Publicación | Estados visibles borrador/publicado; revisión única, borrador separado, cambios pendientes derivados y restauración desde versión activa. | `D-06` | `ADR-0006` (Aceptado), `ADR-0007` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-14`; pruebas de transiciones, concurrencia, restauración y cambios pendientes. | Planificación documentada; diseño detallado de publicación y aceptación de `ADR-0020` pendientes |
| `RF-15` | Publicación y Entrega de notificaciones | Todo cambio visible exige una republicación completa; un grupo inactivo permite republicar solo para destinatarios ya congelados. | `D-06` | `ADR-0007` (Aceptado), `ADR-0008` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-15`; pruebas de versión, cambios relevantes, grupo inactivo, atomicidad y notificación. | Frontera de planificación documentada; diseños detallados de publicación y entrega pendientes |
| `RF-16` | Portal del corredor; Identidad y acceso | Vista móvil del único plan semanal propio, sus fases, bloques, objetivos relativos, modalidad y ubicación, sin exponer datos ajenos. | `D-04`, `D-08` | `ADR-0002`, `ADR-0004`, `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) | Criterios de `RF-16`; pruebas adaptables, de estructura, representación de objetivos y aislamiento. | Modelo de planificación documentado; portal del corredor pendiente |
| `RF-17` | Seguimiento y revisión | Registro único por corredor y entrenamiento, editable durante siete días desde su fecha; `realizado` exige esfuerzo y sensación, `no-realizado` los omite y el comentario admite hasta `1.000` caracteres solo tras consentimiento explícito y separado. | `D-07` | `ADR-0004` (Aceptado), `ADR-0009` (Aceptado), `ADR-0010` (Aceptado) | Criterios de `RF-17`; pruebas de valores, consentimiento y retirada del comentario, ventana, pertenencia, actualización y concurrencia. | Pendiente |
...
| --- | --- | --- |
| `D-01` | Taxonomías, segmentos solapables, grupos exclusivos, reconfiguración atómica, excepciones e instantáneas de versión y destinatarios efectivos. | `ADR-0005` (Aceptado), `ADR-0006` (Aceptado), `ADR-0007` (Aceptado), `ADR-0019` (Aceptado), `ADR-0020` (Propuesto) |
| `D-02` | Modalidad del corredor dentro de la taxonomía controlada y modalidad propia del entrenamiento sin duplicar clasificación. | `ADR-0005` (Aceptado), `ADR-0019` (Aceptado), `ADR-0020` (Propuesto) |
...
| `D-04` | Ubicación libre opcional por entrenamiento con modalidad presencial explícita. | `ADR-0006` (Aceptado), `ADR-0020` (Propuesto) |
| `D-05` | Gramática limitada de reglas de segmentos. | `ADR-0005` (Aceptado), `ADR-0019` (Aceptado) |
| `D-06` | Borrador con revisión global, republicación atómica, versiones completas, destinatarios congelados y solicitud transaccional de correo. | `ADR-0007` (Aceptado), `ADR-0008` (Aceptado), `ADR-0011` (Aceptado), `ADR-0020` (Propuesto) |
...
| `ADR-0020`: ciclo de vida, objetivos e historial de planificación | Grupos activos e inactivos, reconfiguración multigrupo, borradores, objetivos, modalidad, historial y purga. | Implementación de `planning`; datos personales reales y producción. | Revisor de arquitectura; Revisor de privacidad o DPO antes de datos reales | Propuesto con decisiones de producto resueltas; pendiente de validación y aceptación explícitas. |

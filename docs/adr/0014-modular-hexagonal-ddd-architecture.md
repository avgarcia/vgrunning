...
> **Refinamiento aceptado:** `ADR-0021` mantiene este mapa de dependencias mediante un puerto de elegibilidad definido por `notification-delivery` e implementado por `publication`, que consulta `runner-management` antes de un intento de correo. No se añade una dependencia desde entrega hacia módulos de negocio.

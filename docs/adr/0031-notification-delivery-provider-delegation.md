...
- **Deuda aceptada:** la semántica exacta del `idempotencyKey` de Brevo en `POST /v3/smtp/email` (frente al envío por lotes, donde sí está documentada) no está confirmada por prueba propia. Si resultase que solo correlaciona sin suprimir el duplicado físico, la política pasa a fallo cerrado (ver diseño detallado, sección *Reintentos y resultados inciertos*) sin requerir otro ADR.
...
- **Bloqueante para producción:** confirmar con prueba sintética la semántica de `idempotencyKey` en envío individual. Responsable: Revisor de arquitectura. Tratamiento: si no suprime el duplicado, activar la política de fallo cerrado antes de habilitar Brevo con datos reales.

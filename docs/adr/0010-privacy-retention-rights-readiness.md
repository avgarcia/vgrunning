...
`ADR-0018` aceptado reemplaza como decisión de arquitectura el plazo de `30` días de cuenta, perfil y clasificación del corredor por una conservación automática restringida de `24` meses para facilitar reactivación. La aplicación de ese plazo a datos personales reales sigue bloqueada hasta que una revisión especializada confirme su base, necesidad y proporcionalidad; mientras tanto, el desarrollo y las pruebas usarán exclusivamente datos ficticios, sintéticos o anonimizados de forma irreversible.
...
Al terminar la relación se desactivará inmediatamente el acceso. La política general de este ADR mantiene los `30` días; para cuenta, perfil y clasificación del corredor, `ADR-0018` la reemplaza arquitectónicamente por `24` meses, condicionados antes de datos reales a la revisión especializada allí indicada. El resto seguirá su plazo propio. Cualquier conservación identificable posterior deberá estar respaldada por la finalidad y base aprobadas y tendrá acceso restringido.

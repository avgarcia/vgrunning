...
> **Evolución propuesta:** `ADR-0021` conserva una solicitud por versión y destinatario congelado, pero introduce la omisión terminal del intento cuando el corredor no está `active` justo antes del proveedor. La baja no reescribe el destinatario histórico.
